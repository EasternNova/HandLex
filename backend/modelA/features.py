from __future__ import annotations

import math

import numpy as np


def _xyz(landmarks):
    """
    Convert MediaPipe's 21 hand landmarks into
    a (21, 3) float32 NumPy array.

    Each landmark contains:
        x
        y
        z
    """

    if landmarks is None:
        raise ValueError(
            "Cannot convert None landmarks to XYZ."
        )

    if len(landmarks) != 21:
        raise ValueError(
            f"Expected 21 hand landmarks, got {len(landmarks)}."
        )

    return np.array(
        [
            [lm.x, lm.y, lm.z]
            for lm in landmarks
        ],
        dtype=np.float32,
    )


def normalize_landmarks(landmarks):
    """
    V2 normalization.

    Steps:
        1. Move the wrist (landmark 0) to the origin.
        2. Scale the hand using the maximum
           wrist-to-landmark distance.

    Returns
    -------
    np.ndarray
        Shape: (21, 3)
    """

    xyz = _xyz(landmarks)

    wrist = xyz[0].copy()

    # Translate wrist to origin.
    xyz = xyz - wrist

    # Calculate distance of every landmark
    # from the wrist.
    distances = np.linalg.norm(
        xyz,
        axis=1,
    )

    scale = float(
        np.max(distances)
    )

    # Avoid division by zero.
    if scale > 1e-8:
        xyz = xyz / scale

    return xyz.astype(np.float32)


def _angle(a, b, c):
    """
    Calculate angle ABC in radians.

    The middle point B is the vertex.
    """

    ba = a - b
    bc = c - b

    norm_ba = np.linalg.norm(ba)
    norm_bc = np.linalg.norm(bc)

    if norm_ba < 1e-8 or norm_bc < 1e-8:
        return 0.0

    cosine = (
        np.dot(ba, bc)
        / (norm_ba * norm_bc)
    )

    # Numerical safety.
    cosine = np.clip(
        cosine,
        -1.0,
        1.0,
    )

    return float(
        math.acos(cosine)
    )


def engineered_features(landmarks):
    """
    Create the V3 geometric feature representation.

    V3 consists of:

        63 normalized XYZ coordinates
        + 10 finger joint angles
        + 20 wrist-to-landmark distances

    Total:
        93 geometric features
    """

    xyz = normalize_landmarks(
        landmarks
    )

    # --------------------------------------------------------
    # Finger joint angle definitions
    # --------------------------------------------------------

    angle_triplets = [
        # Thumb
        (0, 1, 2),
        (1, 2, 3),
        (2, 3, 4),

        # Index
        (0, 5, 6),
        (5, 6, 7),
        (6, 7, 8),

        # Middle
        (0, 9, 10),
        (9, 10, 11),
        (10, 11, 12),

        # Ring
        (0, 13, 14),
    ]

    angles = [
        _angle(
            xyz[a],
            xyz[b],
            xyz[c],
        )
        for a, b, c in angle_triplets
    ]

    # --------------------------------------------------------
    # Wrist-to-landmark distances
    # --------------------------------------------------------

    wrist = xyz[0]

    distances = [
        float(
            np.linalg.norm(
                xyz[i] - wrist
            )
        )
        for i in range(1, 21)
    ]

    # --------------------------------------------------------
    # Combine geometric features
    # --------------------------------------------------------

    return np.concatenate(
        [
            xyz.flatten(),
            np.asarray(
                angles,
                dtype=np.float32,
            ),
            np.asarray(
                distances,
                dtype=np.float32,
            ),
        ]
    ).astype(np.float32)


def make_feature_vector(
    landmarks=None,
    handedness=None,
    version=1,
):
    """
    Create the feature vector used by ModelA.

    Parameters
    ----------
    landmarks:
        MediaPipe's 21 hand landmarks.

        None means no hand was detected.

    handedness:
        MediaPipe handedness label.

        Examples:
            "Right"
            "Left"

    version:
        1 = raw MediaPipe x/y/z coordinates
            63 features

        2 = wrist-relative + scale-normalized
            x/y/z coordinates
            63 features

        3 = V2 + angles + distances
            + hand-present
            + handedness
            95 features

    Returns
    -------
    np.ndarray
        Feature vector.
    """

    # --------------------------------------------------------
    # NO HAND
    # --------------------------------------------------------

    if landmarks is None:

        if version == 3:
            # 93 geometric features
            # + 1 hand-present feature
            # + 1 handedness feature
            return np.zeros(
                95,
                dtype=np.float32,
            )

        return np.zeros(
            63,
            dtype=np.float32,
        )

    # --------------------------------------------------------
    # V1
    # --------------------------------------------------------

    if version == 1:

        return (
            _xyz(landmarks)
            .flatten()
            .astype(np.float32)
        )

    # --------------------------------------------------------
    # V2
    # --------------------------------------------------------

    if version == 2:

        return (
            normalize_landmarks(
                landmarks
            )
            .flatten()
            .astype(np.float32)
        )

    # --------------------------------------------------------
    # V3
    # --------------------------------------------------------

    if version == 3:

        geometric = engineered_features(
            landmarks
        )

        # Hand is present.
        hand_present = np.array(
            [1.0],
            dtype=np.float32,
        )

        # Preserve the existing ModelA
        # handedness encoding:
        #
        # Right = 1
        # Left  = 0
        #
        # Unknown = 0
        handedness_value = 0.0

        if handedness is not None:

            try:
                name = str(
                    handedness
                ).lower()

                if "right" in name:
                    handedness_value = 1.0

                elif "left" in name:
                    handedness_value = 0.0

            except Exception:
                pass

        handedness_feature = np.array(
            [handedness_value],
            dtype=np.float32,
        )

        features = np.concatenate(
            [
                geometric,
                hand_present,
                handedness_feature,
            ]
        ).astype(np.float32)

        if features.shape[0] != 95:
            raise RuntimeError(
                "V3 feature construction error: "
                f"expected 95 features, "
                f"got {features.shape[0]}"
            )

        return features

    # --------------------------------------------------------
    # Unsupported version
    # --------------------------------------------------------

    raise ValueError(
        f"Unsupported feature version: {version}. "
        "Expected 1, 2, or 3."
    )